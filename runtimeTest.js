//Import dataset
console.log("Hello")
var dataset = d3.csv("erasmus_test.csv", function(data) {
    for (var i = 0; i < 9; i++) {
        console.log(data["GroupLeader"])
    }
});
console.log("behind dataset");
//console.log(dataset[2].GroupLeader);
...
var tmp = ['2','3']
tmp = dataset["Mobility Duration"];
tmp = tmp.slice(0,10)
var duration = tmp.map(Number);
...
var heightScalingFactor = chartHeight / getMax(duration)
...
svg
  ...
  .data(duration)                                              // Then I'm mapping the dataset to those rectangles.
